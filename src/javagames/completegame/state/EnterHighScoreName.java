...
public class EnterHighScoreName extends AttractState {
	...
	public void processInput(float delta) {
		if (keys.keyDownOnce(KeyEvent.VK_A)) {
			selectedCol = roll(selectedCol - 1, 0, colCount - 1);
			resetHeldKey();
		}
		if (keys.keyDown(KeyEvent.VK_A) && updateHeldKey()) {
			selectedCol = roll(selectedCol - 1, 0, colCount - 1);
		}
		if (keys.keyDownOnce(KeyEvent.VK_D)) {
			selectedCol = roll(selectedCol + 1, 0, colCount - 1);
			resetHeldKey();
		}
		if (keys.keyDown(KeyEvent.VK_D) && updateHeldKey()) {
			selectedCol = roll(selectedCol + 1, 0, colCount - 1);
		}
		...
		if (keys.keyDown(KeyEvent.VK_W) && updateHeldKey()) {
			selectedRow = roll(selectedRow - 1, 0, rowCount - 1);
		}
		...
		if (keys.keyDown(KeyEvent.VK_S) && updateHeldKey()) {
			selectedRow = roll(selectedRow + 1, 0, rowCount - 1);
		}
